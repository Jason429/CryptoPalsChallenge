...
f = open("/home/jason/Projects/Crypto/Set1/detectSingleXOR.txt")
s = f.read()
f.close()
# Create two copies (one to remove newlines and one to keep them)
ss = s
s = s.strip('\n')
...
def xorsingle(msg,xor,best=3):
    ...
    # Create var to hold possible XORs
    test = []
...
def xormsg(msg,xor):
    ...
    for i in range(len(xor)):
        result = bytearray(len(msg))
        for j in range(len(msg)):
            if type(xor) == type(bytes(b't')):
                result[j] = msg[j] ^ int.from_bytes(xor[i],"big")
            else:
                if type(xor) == type([]):
                    result[j] = msg[j] ^ int(xor[i])
        result = bytes(result)
