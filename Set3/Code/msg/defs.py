...
def single(msg, xorIter, char_weight=1, eng_weight=1, x=1):
    """
...
def array_quality(binary, test_string=None):
    """
...
"""
    ...
    b_string = bytearray(test, encoding='utf8')
    size = len(binary)
    test_size = len([x for x in binary if x in b_string])
